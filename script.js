...
watermark.style =
  "color:white;opacity:.3;font-size:10px;position:fixed;bottom:25px;left:25px;z-index:2";
...
function multifungsi() {
  if (aktigeser == 1) {
    ...
    if (thisgeser == totalPesan) {
      setTimeout(pesanngetik, 300);
      setTimeout(initerakhir, 300);
    }
  }
  if (thisgeser == 100) {
    Tombol.style = "";
...
async function aksibalas() {
  await swals.fire("OK!", "Kirim pesan ke WA aku, ya!", "success");
  window.location =
    "https://api.whatsapp.com/send?phone=" + nomorhp + "&text=" + pesanwhatsapp;
  Tombol.style = "opacity:1;transform: scale(1)";
}
